...
export const stratagems: Record<Stratagem['id'], Stratagem> = (() => {
  const strats = {
    reinforce: {
      id: 'reinforce',
      title: 'Reinforce',
  ...
  const codes = ['w', 'a', 's', 'd'];
  for (let key of Object.keys(strats) as Stratagem['id'][]) {
    let code = '';
    const codeLength = Math.floor(Math.random() * 7) + 3;
    for (let j = 0; j < codeLength; j++) {
      code += codes[Math.floor(Math.random() * codes.length)];
    }
    (strats as any)[key].code = code;
  }
  return strats;
})();
